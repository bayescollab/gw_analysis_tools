 ...
 * inspiral at the requested frequencies using the pyEFPE SUA. Sets tolerances
 * with pyefpe::ParameterPreset::Production.
 *
 * \param frequencies Array of gravitational-wave frequencies in Hz
 * \param length Number of elements in \p frequencies
 * \param wp Output polarizations; hplus and hcross must be pre-allocated to \p
 * length
 * \param params Source parameters — masses in solar masses, spins
 * dimensionless, distance in Mpc, \c e_start and \c mean_anomaly_start for
 * eccentricity
 *
 * \return 1 on success
 *
...
/*! \brief Uniform-grid frequency-domain waveform for the EFPE
 * eccentric-precessing model.
 *
 * Identical to efpe_fourier_waveform() but uses the pyEFPE uniform-grid
 * generator, which avoids per-frequency allocation and is the preferred path
 * for where the analysis band is a fixed, evenly-spaced grid. The caller is
 * responsible for ensuring that \p frequencies is uniform; no check is
 * performed.
 *
 * \param frequencies Uniform array of gravitational-wave frequencies in Hz;
 *                    f_min = frequencies[0], delta_f = frequencies[1] -
 * frequencies[0]
 * \param length Number of elements in \p frequencies
 * \param wp Output polarizations; hplus and hcross must be pre-allocated to \p
 * length
...
/*! \brief Frequency-domain waveform for the EFPE model using the FastProduction
 * preset.
 *
 * Identical to efpe_fourier_waveform() but applies
 * pyefpe::ParameterPreset::FastProduction (sua_kmax=2,
 * amplitude_interp_nodes=8), which gives ~1.8x speedup over Production with
 * <2e-4 relative logL error across the LISA circular validation suite.
 */
template <class T>
...
/*! \brief Uniform-grid frequency-domain waveform for the EFPE model using the
 * FastProduction preset.
 *
 * Identical to efpe_fourier_waveform_uniform() but applies
 * pyefpe::ParameterPreset::FastProduction (sua_kmax=2,
 * amplitude_interp_nodes=8).
 */
template <class T>
...
template <>
int efpe_fast_fourier_waveform_uniform<double>(
