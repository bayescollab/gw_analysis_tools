...
void read_LOSC_data_file(std::string filename, double* output,
                         double* data_start_time, double* duration, double* fs);

void read_LOSC_PSD_file(std::string filename, double** output, int rows,
                        int cols);

void allocate_LOSC_data(std::string* data_files, std::string psd_file,
                        int num_detectors, int psd_length, int data_file_length,
                        double trigger_time, double post_merger_duration,
                        std::complex<double>** data, double** psds,
                        double** freqs);
...
void read_file(
    ...
    std::vector<std::vector<T>>&
        output, /*< output to store file read-in, passed in by reference*/
  ...
  if (file_in.good()) {  // Checks if the file was read in successfully with no
    ...
    while (std::getline(file_in, line)) {  // Reads the current line in the file
      ...
      T item;  // Initializes to read an item of value type T (based on the
               // vector that is passed in)
      while (lineStream >> item) {  // Parses for itmes until a new line is read
        row.push_back(
            item);  // Adds the currently parsed item to the row vector
        if (lineStream.peek() ==
            delimiter)  // If the next item is the delimiter, ignore it
        {
          lineStream.ignore();
        }
      }
