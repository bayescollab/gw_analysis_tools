 ...
 *
 * IMRPhenomD sky-averaged (7) -- ln A0, phic, tc, ln chirpmass, ln eta,
 * chi_symm, chi_antisymm
 *
 * ppE_IMRPhenomD_Inspiral sky-averaged (7 + mods)-- ln A0, phic, tc, ln
 * chirpmass, ln eta, chi_symm, chi_antisymm, betas
 *
 * ppE_IMRPhenomD_IMR sky-averaged (7 + mods)-- ln A0, phic, tc, ln chirpmass,
 * ln eta, chi_symm, chi_antisymm, betas
 *
 * IMRPhenomD !sky_averaged (11) --  RA, DEC, psi, phiRef, tc, \iota_L, ln DL,
 * ln chirpmass, eta, chi1, chi2
 *
 * ppE_IMRPhenomD_Inspiral !sky_averaged (11+mods) -- RA, DEC, psi, phiRef, tc,
 * \iota_L, ln DL, ln chirpmass, eta, chi1, chi2, betas
 *
 * ppE_IMRPhenomD_IMR !sky_averaged (11+mods) -- RA, DEC, psi, phiRef, tc,
 * \iota_L, ln DL, ln chirpmass, eta, chi1, chi2, betas
 *
 * dCS_IMRPhenomD !sky_averaged (12) -- RA, DEC, psi, phiRef, tc, \iota_L, ln
 * DL, ln chirpmass, eta, chi1, chi2, \alpha^2 (sec^4)
 *
 * EdGB_IMRPhenomD !sky_averaged (12) --RA, DEC, psi, phiRef, tc, \iota_L, ln
 * DL, ln chirpmass, eta, chi1, chi2, \alpha^2 (sec^4)
 *
 * IMRPhenomPv2 !sky_averaged (13) --  RA, DEC, psi,phiRef, tc, \iota_L,  ln DL,
 * ln chirpmass, eta, chi_1 \dot \hat{L}, chi_2 \dot \hat{L} ,chi_p,  \phi_p
 * (all at f_ref)
 *
 * ppE_IMRPhenomPv2_Inspiral !sky_averaged (13 + mods) --  RA, DEC, psi,phiRef,
 * tc, \iota_L, ln DL, ln chirpmass, ln eta, chi_1 \dot \hat{L}, chi_2 \dot
 * \hat{L} ,chi_p,  \phi_p, \vec{beta} (all at f_ref)
 *
 * ppE_IMRPhenomPv2_IMR !sky_averaged (13 + mods) --  RA, DEC, psi, phiRef, tc,
 * \iota_L, ln DL, ln chirpmass, ln eta, chi_1 \dot \hat{L}, chi_2 \dot \hat{L}
 * ,chi_p,  \phi_p, \vec{beta} (all at f_ref)
 *
 * dCS_IMRPhenomPv2 !sky_averaged (14) --  RA, DEC, psi, phiRef, tc, \iota_L, ln
 * DL, ln chirpmass, ln eta, chi_1 \dot \hat{L}, chi_2 \dot \hat{L} ,chi_p,
 * \phi_p,  \alpha (all at f_ref)
 *
 * EdGB_IMRPhenomPv2 !sky_averaged (14) --  RA, DEC, psi, phiRef, tc, \iota_L,
 * ln DL, ln chirpmass, ln eta, chi_1 \dot \hat{L}, chi_2 \dot \hat{L} ,chi_p,
 * \phi_p,  \alpha (all at f_ref)
 *
 * IMRPhenomPv3 !sky_averaged (13) --  RA, DEC, psi,phiRef, tc, \iota_L,  ln DL,
 * ln chirpmass, eta, chi_1 \dot \hat{L}, chi_2 \dot \hat{L} ,chi_p,  \phi_p
 * (all at f_ref)
 *
 * All MCMC options correspond to the base, minus the coalescence time (which is
 * maximized over) -- Reduced MCMC option correspond to the options above minus
 * t_c (and phic for sky_averaged) -- Non reduced correspond to replacing \chi_1
 * \dot \hat{L}, \chi_2 \dot \hat{L}, \chi_p and \phi_p with |\chi_1|, |\chi_2|,
 * \theta_1, \theta_2, \phi_1, and \phi_2
 *
 * Both fisher functions are compatible with OpenMP, but not with pthreads (if
 * ADOL-C was installed with the openmp option)
 *
 * NOTE about non-sky-averaged: Matrices will most likely be singular if only
 * using one detector, and still largly singular with 2. 3 detectors will mostly
 * guarantee that the matrices are invertible.
 */

/*!\brief Calculates the fisher matrix for the given arguments
 *
 * Utilizes numerical derivatives -- non-skyaveraged supports up to 4th order
 * finite difference (sky averaged supports second order only)
 *
 ...
 * numerical error using automatic differention in ``batch'' mode for
 * modifications to GR
 *
 * Built  around  ADOL-C -- A. Walther und A. Griewank: Getting started with
 * ADOL-C. In U. Naumann und O. Schenk, Combinatorial Scientific Computing,
 * Chapman-Hall CRC Computational Science, pp. 181-202 (2012).
 *
 * This constructs the fisher for a list of modifications in the usual way, but
 * skips elements of the fisher that correspond to (mod, mod) elements.
 * Specifically, it calculates all the fisher elements except (i, j )
 * i>GR_dimension && j > GR_dimension && i!=j.
 *
 * To find the fisher for one of the modifications, simply remove all the other
 * dimensions associated with the extra modifications using rm_fisher_dim in
 * util.h
 *
 * !NOTE!:This routine only works as intended when GR is the injected value, ie,
 * all the  betas are evaluated at 0. And since the covariances between
 * modifications are not computed, this should only be used to look at one
...
/*! \brief Calculates the fisher matrix for the given arguments to within
 * numerical error using automatic differention - slower than the numerical
 * version
 *
 * Build  around  ADOL-C -- A. Walther und A. Griewank: Getting started with
 * ADOL-C. In U. Naumann und O. Schenk, Combinatorial Scientific Computing,
 * Chapman-Hall CRC Computational Science, pp. 181-202 (2012).
 *
 * Note: If trying to run large number of Fishers concurrently with openmp, you
 * may need to change some ADOL-C parameters. Copy the example .adolcrc file in
 * data/ to the working directory and adjust as needed
 *
 * If integration method is gaussian quadrature and the frequencies are !NULL,
 * the weights must be pre-allocated.
 *
 ...
 * 	GAUSSLEG
 *
 * log10_f used if frequencies are logarithmically (base 10) spaced. NOTE: not
 * sure if this would work for SIMPSONS -- this hasn't been tested. For now,
 ...
 * differentiation
 *
 * Possibly slower than the numerical derivative, but not susceptible to
 * truncation error from finite difference
 *
 * Higher dimensional fishers actually could be faster
 *
 * NOTE: dimension parameter ALWAYS refers to the dimension of the fisher (ie
 * the length of the source parameter vector), even though the derivatives are
 * computed wrt dimension +1 or dimension + 2 -- the +1(+2) are for the
 * frequency deriv(time deriv)
 *
 * autodiff_time_deriv indicates if the time derivatives for space detectors
 * should be done, either with the full autodiff hessian (slow) or with the
 ...
 *
 * If specific derivatives need to taken, take this routine as a template and
 * write it yourself.
 *
 * The dt array has shape [dimension+1][length] (dimension + 1 for the frequency
 * derivative, so dimension should only include the source parameters)
 *
 * This takes the autodiff derivative wrt source parameters, and a numerical
 * derivative for the frequency
 *
 */
 ...
 *
 * This is kept separate to improve the modularity of the code. Waveform
 ...
 *
 * This is one of the places where the generation-method/dimension/sky_average
 ...
 *
 * Skips elements that have dimensions (i,j) for i!=j && i>base_dim &&
 * j>base_dim
 *
 ...
 * Trade offs:
 *
 * Every element is calculated independently, so no information is retained
 * between elements. In the brute force calculation, there is information
 * reused.
 *
 * However, time can be saved by spending less time on trivial elements
 * (identically 0 elements, etc) and better spent on complicated elements
 *
 * Does not have a direct interpretation in terms of integration time, as the
 * scheme is adaptative. Sampling frequency and integration time are ``as good
 * as they need to be'' to calculate the fisher
 *
 ...
 *
 * d \theta_i / d \theta_j where \theta_i is the ppE parameterrs, and \theta_j
 ...
 *
 * The beta value in the input parameter structure must be the value of the
 ...
 * See ppE_utilities.cpp for a list of supported theories.
 *
 * The covariance matrix is defined as (\Gamma)^-1 where, \Gamma is the Fisher
 * matrix
 */
