 ...
 * 	[type] Parameter name = val
 *
 * The unordered_maps contain the parameter names and are mapped to the value
 * val
 *
...
/*! \brief Takes in the shorthand for primitive datatypes in c++ and returns
 * code based on type
 *
 * Returns:
 *
...
void read_file(
  ...
  for (i = 0; i < rows; i++) {
    for (j = 0; j < cols; j++) output[i][j] = temp[cols * i + j];
  }
  free(temp);
}
...
void read_file(
    std::string filename, /**< input filename, relative to execution directory*/
    double* output /**<[out] output array, assumed to have the proper length of
                      total items*/
) {
  std::fstream file_in;
  file_in.open(filename, std::ios::in);
...
void read_file(
    std::string filename, /**< input filename, relative to execution directory*/
    int* output /**<[out] output array, assumed to have the proper length of
                   total items*/
) {
  std::fstream file_in;
  file_in.open(filename, std::ios::in);
 ...
 */
void write_file(std::string filename, /**<Filename of output file, relative to
                                         execution directory*/
...
void write_file(std::string filename, /**<Filename of output file, relative to
  ...
  out_file.precision(15);
  if (out_file) {
    for (int j = 0; j < length; j++) out_file << input[j] << std::endl;
    out_file.close();
  } else {
    std::cout << "ERROR -- Could not open file" << std::endl;
  }
 ...
 * Convenience function for cutting off the first few lines of text
 */
void read_LOSC_PSD_file(std::string filename, double** output, int rows,
                        int cols) {
  std::fstream file_in;
  file_in.open(filename, std::ios::in);
  std::string line;
 ...
 *
 * Trims data for Tobs (determined by PSD file) 3/4*Tobs in front of trigger,
 * and 1/4*Tobs behind or max 2 seconds on the end
 *
 * Currently, default to sampling frequency and observation time set by PSD --
 * cannot be customized
 *
 * Output is in order of PSD columns -- string vector of detectos MUST match
 * order of PSD cols
 *
 * Output shapes--
 *
...
void allocate_LOSC_data(
    ...
    int data_file_length, /**< Length of the data file (number of rows of
                             DATA)*/
  ...
  for (int i = 0; i < num_detectors; i++) {
    read_LOSC_data_file(data_files[i], temp_data[i], &file_start, &duration,
                        &fs);
  }
  debugger_print(__FILE__, __LINE__,
                 "Trigger time: " + std::to_string(trigger_time));
  debugger_print(__FILE__, __LINE__,
                 "File initial GPS: " + std::to_string(file_start));
  debugger_print(__FILE__, __LINE__,
                 "Data file duration: " + std::to_string(duration));
  debugger_print(__FILE__, __LINE__,
                 "Data file sampling frequency: " + std::to_string(fs));
  ...
  if (time_start < file_start || time_end > (file_start + duration)) {
    debugger_print(__FILE__, __LINE__,
                   "ERROR -- trigger time does not fit inside data file!");
    return;
  }

  debugger_print(__FILE__, __LINE__,
                 "Signal Duration: " + std::to_string(Tobs));
  debugger_print(__FILE__, __LINE__,
                 "Start,End: " + std::to_string(time_start) + "," +
                     std::to_string(time_end));
  ...
  std::complex<double>** fft_data = (std::complex<double>**)malloc(
      sizeof(std::complex<double>*) * num_detectors);
  for (int i = 0; i < num_detectors; i++) {
    fft_data[i] =
        (std::complex<double>*)malloc(sizeof(std::complex<double>) * N_trimmed);
 ...
 */
void free_LOSC_data(std::complex<double>** data, double** psds, double** freqs,
                    int num_detectors, int length) {
  deallocate_2D_array(psds, num_detectors, length);
  deallocate_2D_array(freqs, num_detectors, length);
  for (int i = 0; i < num_detectors; i++) free(data[i]);
  free(data);
}
...
/*! \brief Takes in LOSC PSD file and returns the number of data elements in
 * file
 */
...
/*! \brief Takes in LOSC data file and returns the number of data elements in
 * file
 */
 ...
 */
void transpose_data_to_column_major(
  ...
  std::vector<std::vector<double>> converted_data;

  // Grab size information
  size_t num_rows = file_data.size();
  size_t num_cols = file_data[0].size();

  converted_data.resize(num_cols, std::vector<double>(num_rows));

  // Perform the transposition
  for (size_t i = 0; i < num_rows; ++i) {
    for (size_t j = 0; j < num_cols; ++j) {
      converted_data[j][i] = file_data[i][j];
    }
  }
...
//		this->DSET = new
// H5::DataSet(this->DFILE.createDataSet(DSET_NAME,
// H5::PredType::NATIVE_DOUBLE,this->DSPACE, *(this->plist)));
//	}
...
//			//this->pimpl->allocate(dims_outer, dims_inner,
// chunkdims, DFILE_NAME,DSET_NAME,deflate);
// }
