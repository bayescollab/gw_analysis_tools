...
void MCMC_fisher_wrapper(bayesship::positionInfo *pos,   double **output, void *userParameters);

void explicit_marginalization(bayesship::positionInfo *pos,   double **output, std::vector<int> ids, void *userParameters);

int invertFisherBlock(double **fisherIn, double **fisherOut, int dimIn, std::vector<int> ids);

void MCMC_fisher_wrapper_RJ(bayesship::positionInfo *pos,   double **output, std::vector<int> block, void *userParameters);
...
public:
	...
	virtual double eval(bayesship::positionInfo *pos, int chainID)
	{
		...
		if(wfExtended){
			pack_local_mod_structure(sampler,pos->parameters, pos->status, mcmcVarRJ->mcmc_generation_method_extended,mcmcVarRJ->user_parameters , mcmcVarRJ->mcmc_mod_struct, &mod_struct_local);
		}
		//#########################################################################
...
public:
	...
	virtual double eval(bayesship::positionInfo *pos, int chainID)
	{
		...
		//#########################################################################
		gen_params_base<double> gen_params;
		std::string local_gen = MCMC_prep_params(pos->parameters, 
...
//##########################################################
	...
	RJPTMCMC_method_specific_prep(generation_method, maxDim,  &(mcmcVarRJ.mcmc_intrinsic), mcmcVarRJ.mcmc_mod_struct);
	//std::cout<<"Extending up to:"<<std::endl;
	//RJPTMCMC_method_specific_prep(generation_method_extended, maxDim,  &(mcmcVarRJ.mcmc_intrinsic), mcmcVarRJ.mcmc_mod_struct);
	...
	//propArray[4] = new bayesship::blockFisherProposal(sampler->ensembleN*sampler->ensembleSize, sampler->minDim, &MCMC_fisher_wrapper_RJ,   sampler->userParameters,  100,sampler,blocks, blockProb );
...
//##########################################################
	...
	PTMCMC_method_specific_prep(generation_method, dimension, &(mcmcVar.mcmc_intrinsic), mcmcVar.mcmc_mod_struct);
	...
	propArray[2] = new bayesship::KDEProposal(sampler->ensembleN*sampler->ensembleSize, sampler->maxDim, sampler, false );
	//propArray[3] = new bayesship::fisherProposal(sampler->ensembleN*sampler->ensembleSize, sampler->maxDim, &MCMC_fisher_wrapper,   sampler->userParameters,  100,sampler);
	if(mcmcVar.mcmc_intrinsic){
		propArray[3] = new bayesship::fisherProposal(sampler->ensembleN*sampler->ensembleSize, sampler->maxDim, &MCMC_fisher_wrapper,   sampler->userParameters,  100,sampler);
	}
	else if(has_substring(generation_method, "EA_IMRPhenomD_NRT") ){
		...
		//				{7,8,9,10}};
		//std::vector<double> blockProb = {1};
		propArray[3] = new bayesship::blockFisherProposal(sampler->ensembleN*sampler->ensembleSize, sampler->minDim, &explicit_marginalization,   sampler->userParameters,  100,sampler,blocks, blockProb );
	...
	////				{7,8,9,10}};
	////std::vector<double> blockProb = {1};
	//propArray[4] = new bayesship::blockFisherProposal(sampler->ensembleN*sampler->ensembleSize, sampler->minDim, &explicit_marginalization,   sampler->userParameters,  100,sampler,blocks, blockProb );
	...
	//bayesship::FisherProposalVariables *fpv = new bayesship::FisherProposalVariables(sampler->ensembleN*sampler->ensembleSize, sampler->maxDim, MCMC_fisher_wrapper,   sampler->userParameters,  100);
...
void explicit_marginalization(bayesship::positionInfo *pos,   double **output, std::vector<int> ids, void *userParameters)
{
	...
	MCMC_fisher_transformations(temp_params, tempOutput,dimension,local_gen,mcmcVar->mcmc_intrinsic,
...
void MCMC_fisher_wrapper(bayesship::positionInfo *pos,   double **output, void *userParameters)
{
	...
	MCMC_fisher_transformations(temp_params, output,dimension,local_gen,mcmcVar->mcmc_intrinsic,
		mcmcVar->mcmc_mod_struct);
