...
public:
	...
	virtual double eval(bayesship::positionInfo *pos, int chainID)
	{
		...
		if(mcmcVar->mcmc_intrinsic){
			if(has_substring(mcmcVar->mcmc_generation_method, "IMRPhenomD") != std::string::npos){
			...
			else if(has_substring(mcmcVar->mcmc_generation_method, "IMRPhenomP")!=std::string::npos){
...
static void cleanup_gen_params_rb(const std::string &local_gen,
    gen_params_base<double> &gp, MCMC_modification_struct *ms)
{
