...
bool tidal_love_boundary_violation(double q, double lambda_s) {
  ...
  if (q < 1.2321 - .124616 * log(lambda_s)) {
    return true;
  }
...
double EA_current_constraints(bayesship::positionInfo* position,
   ...
   */
  if (sp.cw_EA < (-sp.csigma_EA / (1. - sp.csigma_EA))) {
    return bayesship::limitInf;
  }
  ...
  if (sp.cTsq_EA < 0 || sp.cVsq_EA < 0 || sp.cSsq_EA < 0) {
    return bayesship::limitInf;
  }
  ...
  if (sp.s1_EA >= 1 || sp.s2_EA >= 1) {
    return bayesship::limitInf;
  }

  // if(fabs(sp.alpha1_EA) > pow(10, -4.) || fabs(sp.alpha2_EA) > 4.*pow(10,
  // -7.)){return bayesship::limitInf;}
  /* Throws out points that do not obey observational solar system constraints
   * on alpha1 and alpha2 arXiv:1403.7377 and arXiv:gr-qc/0509114
   */
  ...
  if (Cherenkov) {
    bool violate = false;
    if (sp.cV_EA < 1) {
      if (abs(sp.c13_EA * sp.c13_EA *
              (sp.c1_EA * sp.c1_EA + 2 * sp.c1_EA * sp.c3_EA +
               sp.c3_EA * sp.c3_EA - 2 * sp.c4_EA) /
              (2 * sp.c1_EA * sp.c1_EA)) >= 7 * pow(10, -32.))
    ...
    if (sp.cS_EA < 1) {
      if (fabs((sp.c2_EA + sp.c3_EA - sp.c4_EA) / sp.c1_EA) > pow(10, -22.)) {
        if ((sp.c3_EA - sp.c4_EA) * (sp.c3_EA - sp.c4_EA) / fabs(sp.c14_EA) >=
            pow(10, -30.)) {
          return bayesship::limitInf;
        }
      ...
      if (fabs((sp.c4_EA - sp.c2_EA - sp.c3_EA) / sp.c1_EA) >=
          3 * pow(10, -19.)) {
        return bayesship::limitInf;
      }
...
double logPriorStandard_D_mod::eval(bayesship::positionInfo* position,
  ...
  for (int i = initial_nongr_id; i < dim; i++) {
    if (pos[i] < PD->mod_priors[i - initial_nongr_id][0] ||
        pos[i] > PD->mod_priors[i - initial_nongr_id][1]) {
      return bayesship::limitInf;
    }
  }
...
double logPriorStandard_D_NRT_EA::eval(bayesship::positionInfo* position,
  ...
  double NS = logPriorStandard_D_NRT::eval(position, chainID);
  if (NS == bayesship::limitInf) {
    return bayesship::limitInf;
  }
  double EA_constraints = EA_current_constraints(position, PD);
  if (EA_constraints == bayesship::limitInf) {
    return bayesship::limitInf;
  }
...
double logPriorStandard_D_NRT::eval(bayesship::positionInfo* position,
  ...
  if (PD->tidal_love) {
    if (exp(pos[11]) < PD->tidal_s_prior[0] ||
        exp(pos[11]) > PD->tidal_s_prior[1]) {
      return bayesship::limitInf;
    }
    if (tidal_love_boundary_violation(q, exp(pos[11]))) {
      return bayesship::limitInf;
    }
    factor += pos[11];

  } else {
    if (exp(pos[11]) < PD->tidal1_prior[0] ||
        exp(pos[11]) > PD->tidal1_prior[1]) {
      return bayesship::limitInf;
    }
    if (exp(pos[12]) < PD->tidal2_prior[0] ||
        exp(pos[12]) > PD->tidal2_prior[1]) {
      return bayesship::limitInf;
    }
    factor += pos[11];
    factor += pos[12];
...
double logPriorStandard_D::eval(bayesship::positionInfo* position,
  ...
  if (m2 < PD->mass2_prior[0] || m2 > PD->mass2_prior[1]) {
    return bayesship::limitInf;
  }
  // ###########
...
if (std::exp(pos[6]) < PD->DL_prior[0] || std::exp(pos[6]) > PD->DL_prior[1]) {
  return bayesship::limitInf;
}  // DL
}  // DL
...
double logPriorStandard_D_NRT_EOS::eval(bayesship::positionInfo* position,
  ...
  // double m1 = calculate_mass1(chirp,eta );
  // double m2 = calculate_mass2(chirp,eta );
  if (pos[7] < PD->nbc1_prior[0] || pos[7] > PD->nbc1_prior[1]) {
    return bayesship::limitInf;
  }
  if (pos[8] < PD->nbc2_prior[0] || pos[8] > PD->nbc2_prior[1]) {
    return bayesship::limitInf;
  }
  // ###########
  if ((pos[0]) < PD->RA_bounds[0] || (pos[0]) > PD->RA_bounds[1]) {
...
double logPriorStandard_P_NRT::eval(bayesship::positionInfo* position,
  ...
  if (PD->tidal_love) {
    ...
    if (exp(pos[15]) < PD->tidal1_prior[0] ||
        exp(pos[15]) > PD->tidal1_prior[1]) {
      return bayesship::limitInf;
    }
    if (exp(pos[16]) < PD->tidal2_prior[0] ||
        exp(pos[16]) > PD->tidal2_prior[1]) {
      return bayesship::limitInf;
    }
    factor += pos[15];
    factor += pos[16];
