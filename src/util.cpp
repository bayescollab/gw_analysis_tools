...
/*! \file
 *
 * General utilities that are not necessarily specific to any part of the
 * project at large
 */
...
int mvn_sample(int samples, double* mean, double** cov, int dim,
...
/*! \brief Samples from a multivariate-gaussian with covariance cov and mean
 * mean, with dimension dim and puts the output in output Taken from blogpost:
 * https://juanitorduz.github.io/multivariate_normal/
 */
int mvn_sample(int samples, double* mean, double** cov, int dim, gsl_rng* r,
               double** output /**< [out] Size [samples][dim]*/
) {
 ...
 */
void matrix_multiply(double** A, double** B, double** C, int dim1, int dim2,
                     int dim3) {
 ...
 *
 * f_on_fprime is a function pointer that should assign the value result = f(x)
 * / f'(x)
 *
 ...
 */
int newton_raphson_method_1d(void (*f_on_fprime)(double x, double* func,
                                                 double* func_prime,
                                                 void* param),
                             double initial_guess, double tolerance,
                             int max_iterations, void* parameters,
                             double* solution) {
  ...
  do {
    ...
    if (fabs(func_prime) > 1e-12 && !isnan(func) && !isnan(func_prime)) {
...
/*! \brief Function that uses the GSL libraries to interpolate pre-calculated
 * Z-D_L data
 *
 * Initiates the requried functions -- GSL interpolation requires allocating
 * memory before hand
 */
void initiate_LumD_Z_interp(gsl_interp_accel** Z_DL_accel_ptr,
  ...
  *Z_DL_spline_ptr = gsl_spline_alloc(gsl_interp_cspline, npts);
  ...
  data_table.open(
      std::string("GWAT_SHARE_DIR") + "Cosmology_data/tabulated_LumD_Z.csv",
      std::ios::in);
  ...
  if (data_table) {
    while (std::getline(data_table, line)) {
      ...
      while (std::getline(lineStream, item, ',')) {
        if (i < npts) {
          if (j == 0) {
            DLvec[i] = std::stod(item);
          } else if (j == 1) {
            Zvec[i] = std::stod(item);
          }
          j++;
        }
      }
 ...
 */
void free_LumD_Z_interp(gsl_interp_accel** Z_DL_accel_ptr,
                        gsl_spline** Z_DL_spline_ptr) {
 ...
 */
adouble Z_from_DL_interp(adouble DL, gsl_interp_accel* Z_DL_accel_ptr,
                         gsl_spline* Z_DL_spline_ptr) {
  adouble Z = 0;
...
/*! Function that returns Z from a given luminosity Distance -- only Planck15
 */
double Z_from_DL_interp(double DL, gsl_interp_accel* Z_DL_accel_ptr,
                        gsl_spline* Z_DL_spline_ptr) {
  double Z = 0;
  double DLtemp = DL;
  // if(DL>DLvec[npts-1]){
  //	std::cout<<"WARNING: DL exceeded limit: setting to highest value in
  //table"<<std::endl; 	DLtemp=DLvec[npts-1]; 	std::cout<<DL<<std::endl;
 ...
 * folder of the project -- numerically calculated given astropy.cosmology's
 * definitions (http://docs.astropy.org/en/stable/cosmology/) and used
 * scipy.optimize to fit to a power series, stepping in half powers of DL. These
 * coefficients are then output to a header file (D_Z_config.h) which are used
 * here to calculate redshift. Custom cosmologies etc can easily be acheived by
 * editing the python script D_Z_config.py, the c++ functions do not need
 * modification. They use whatever data is available in the header file.
 *
 * 5 cosmological models are available (this argument must be spelled exactly,
 * although case insensitive):
 *
...
double Z_from_DL(double DL, std::string cosmology) {
  ...
  if (cosmo_index == -1) {
    std::cout << "Invalid Cosmology" << std::endl;
    return -1;
  }
 ...
 * folder of the project -- numerically calculated given astropy.cosmology's
 * definitions (http://docs.astropy.org/en/stable/cosmology/) and used
 * scipy.optimize to fit to a power series, stepping in half powers of Z. These
 * coefficients are then output to a header file (D_Z_config.h) which are used
 * here to calculate distance. Custom cosmologies etc can easily be acheived by
 * editing the python script D_Z_config.py, the c++ functions do not need
 * modification. They use whatever data is available in the header file. If the
 * functional form of the fitting function changes, these functions DO need to
 * change.
 *
 * 5 cosmological models are available (this argument must be spelled exactly):
 *
...
double DL_from_Z(double Z, std::string cosmology) {
  ...
  for (int i = 0; i < num_seg; i++) {
    if (Z < boundaries[i + 1]) {
      // if(i == 0 ){
      //	debugger_print(__FILE__,__LINE__,"Boundaries:
      //"+std::to_string(boundaries[i])+" "+std::to_string(boundaries[i+1]) +"
      //"+std::to_string(Z));
      // }
      double* coeffs = new double[interp_deg];
      for (int j = 0; j < interp_deg; j++)
...
T fcontact(T M_detector,         /**< Detector frame total mass in seconds*/
           ...
           std::string cosmology /**< Cosmology to use for getting the redshift
                                    from dl*/
) {
 ...
 *
 */
double cosmology_interpolation_function(double x, double* coeffs,
                                        int interp_degree) {
  double sum = coeffs[0];
 ...
 */
adouble cosmology_interpolation_function(adouble x, double* coeffs,
                                         int interp_degree) {
...
/*! \brief Converts a uniformly random random number into a powerlaw
 * distribution
 *
 * https://mathworld.wolfram.com/RandomNumber.html
 */
double powerlaw_from_uniform(double x0, double x1, double power,
                             double uniform_random_number) {
  return pow((pow(x1, power + 1) - pow(x0, power + 1)) * uniform_random_number +
                 pow(x0, power + 1),
             1. / (power + 1));
}

/*! \brief Calculates a random number from the Maxwell-Boltzmann distribution
 * using 3 gaussian random numbers
 *
 * Maxwell-Boltzmann describes a gaussian distributed vector in 3 space, so the
 * magnitude is the RMS of the components of that vector
 *
 * Assumed mean is at 0
 *
 * Sigma would be equivalent to sqrt(kT/m) in the real maxwell boltzmann
 * distribution
 *
 */
...
/*! \brief Removes a dimension from a matrix (made with fishers in mind, but
 * this is general)
 *
 * Note: the removed_dims list is indexed from 0
 */
void rm_fisher_dim(double** input, int full_dim, double** output,
                   int reduced_dim, int* removed_dims) {
  int colct = 0;
  int rowct = 0;
...
/*! \brief Finds union of two lists which have been pre-sorted. Returns the
 * union, also sorted
 *
 * Allocates memory for C, needs to be deallocated
 *
 */
void vector_union(std::vector<double> A, std::vector<double> B,
                  std::vector<double>* C) {
  auto itA = A.begin();
  auto itB = B.begin();
...
/*! \brief SHOULDN'T BE USED FOR DOUBLESCustom list-interesction implementation
 * for sorted lists
 *
...
// template void
// list_intersect_ptrs<double>(double**,int,double**,int,double**,int *);
// template void
// list_intersect_ptrs<adouble>(adouble**,int,adouble**,int,adouble**,int *);
...
template void list_intersect<double>(double*, int, double*, int, double**,
                                     int*);
...
// template void
// list_intersect_value<double>(double*,int,double*,int,double*,int *); template
// void list_intersect_value<adouble>(adouble*,int,adouble*,int,adouble*,int *);
    ...
    for (int row = 0; row < dim; row++) {
      for (int column = 0; column < dim; column++) {
        if (!std::isnan(gsl_matrix_get(inv, row, column))) {
          ...
          // std::cout<<"Inversion failed -- NAN in inverse matrix -- cholesky
...
/*! \brief Normalize the Fisher matrix before inversion to try and tame
 * singularity issues:
 *
 * arXiv:1108.1826v2 Equation 60-61
 */
int normalized_gsl_cholesky_matrix_invert(double** input, double** inverse,
                                          int dim) {
  int status = 1;
  ...
  if (status == 0) {
    std::cout << "Error: Product of diagonal elements in Fisher normalization "
                 "are negative"
              << std::endl;
    return status;
  }
...
double std_omega(double RA, double std_RA, double std_DEC, double cov_RA_DEC) {
  return 2. * M_PI *
         (std::abs(sin(RA)) * sqrt(std_RA * std_DEC - cov_RA_DEC * cov_RA_DEC));
}

/*! \brief routine to print the progress of a process to the terminal as a
 * progress bar
 *
...
void allocate_FFTW_mem_forward(fftw_outline* plan, int length) {
  ...
  plan->p =
      fftw_plan_dft_1d(length, plan->in, plan->out, FFTW_FORWARD, FFTW_MEASURE);
}
/*! \brief Allocate memory for FFTW3 methods used in a lot of inner products
 * --INVERSE input is a locally defined structure that houses all the pertinent
 * data
 */
void allocate_FFTW_mem_reverse(fftw_outline* plan, int length) {
  plan->in = (fftw_complex*)fftw_malloc(sizeof(fftw_complex) * length);
  plan->out = (fftw_complex*)fftw_malloc(sizeof(fftw_complex) * length);
  plan->p = fftw_plan_dft_1d(length, plan->in, plan->out, FFTW_BACKWARD,
                             FFTW_MEASURE);
}
...
/*! \brief Builds the structure that shuttles source parameters between
 * functions -updated version to incorporate structure argument
 *
 * Populates the structure that is passed to all generation methods - contains
 * all relavent source parameters
 *
...
void source_parameters<T>::populate_source_parameters(
  ...
  /* Convert all dimensionful quantities to seconds and build all needed source
   * quantities once*/
  ...
  // params.chirpmass =
  // (adouble)calculate_chirpmass((double)params.mass1.value(),(double)params.mass2.value());
  this->chirpmass = calculate_chirpmass(this->mass1, this->mass2);
  // params.eta =
  // (adouble)calculate_eta((double)params.mass1.value(),(double)params.mass2.value());
  this->eta = calculate_eta(this->mass1, this->mass2);
  this->M = this->mass1 + this->mass2;
  this->chi_eff =
      (this->mass1 * (this->spin1z) + this->mass2 * (this->spin2z)) / (this->M);
...
//{
//
//	/* Convert all dimensionful quantities to seconds and build all needed
//source quantities once*/ 	source_parameters<T> params; 	params.mass1 =
//param_in->mass1*MSOL_SEC; 	params.mass2 = param_in->mass2*MSOL_SEC;
...
//	params.chi_eff = (params.mass1*(params.spin1z)+
//params.mass2*(params.spin2z))/(params.M); 	params.chi_pn = params.chi_eff -
//(38*params.eta/113)*(2*params.chi_s); 	params.DL =
//param_in->Luminosity_Distance*MPC_SEC; 	params.delta_mass =
//sqrt(1.-4*params.eta); 	params.phiRef = param_in->phiRef; 	params.tc =
//param_in->tc; 	params.sky_average = param_in->sky_average; 	params.A0 =
//A0_from_DL(params.chirpmass, params.DL, params.sky_average); 	return params;
// }
/*! \brief TESTING Simple utility to copy the members of param_in to param_out,
 * for whatever types those are.
 *
 * This is sometimes required to jump from double params to adouble params
 *
 * Memory MUST be allocated for betappe and bppe (if present), and this needs to
 * be deallocated
 *
 */
template <class T, class U>
void transform_parameters(gen_params_base<T>* param_in,
                          gen_params_base<U>** param_out) {
...
/*! \brief Simple utility to copy the members of param_in to param_out, for
 * whatever types those are.
 *
 * This is sometimes required to jump from double params to adouble params
 *
 * Memory MUST be allocated for betappe and bppe (if present), and this needs to
...
template void transform_parameters<double, adouble>(gen_params_base<double>*,
                                                    gen_params_base<adouble>*);
template void transform_parameters<double, adouble>(gen_params_base<double>*,
                                                    gen_params_base<adouble>**);
...
T A0_from_DL(T chirpmass, T DL, bool sky_average) {
  if (sky_average) {
    return sqrt(M_PI / 30) * chirpmass * chirpmass / DL *
           pow(M_PI * chirpmass, -7. / 6);
  } else {
    return sqrt(M_PI * 40. / 192.) * chirpmass * chirpmass / DL *
           pow(M_PI * chirpmass, -7. / 6);
  }
}
...
T DL_from_A0(T chirpmass, T A0, bool sky_average) {
  if (sky_average) {
    return sqrt(M_PI / 30) * chirpmass * chirpmass / A0 *
           pow(M_PI * chirpmass, -7. / 6);
  } else {
    return sqrt(M_PI * 40. / 192.) * chirpmass * chirpmass / A0 *
           pow(M_PI * chirpmass, -7. / 6);
  }
}
...
/*! \brief Builds the structure that shuttles source parameters between
 * functions- outdated in favor of structure argument
 *
 * Populates the structure that is passed to all generation methods - contains
 * all relavent source parameters
 */
...
source_parameters<T> source_parameters<T>::populate_source_parameters_old(
  ...
  params.phiRef = phi_c;
  params.tc = t_c;
  // params.A0 = sqrt(M_PI/30)*params.chirpmass*params.chirpmass/params.DL *
  ...
  if (sky_average) {
    ...
    params.A0 = sqrt(M_PI * 40. / 192.) * params.chirpmass * params.chirpmass /
                params.DL * pow(M_PI * params.chirpmass, -7. / 6);
  }
  return params;
}

/*! \brief Routine to transform from the equatorial coordinate system spherical
 * polar description of the total angular momentum to the detector specific
 * polarization angle and inclination angle (of the total angular momentum)
 *
 * For the terrestrial network, this calculates the polarization angle for a
 * detector at the center of Earth in equatorial coordinates, which is the
 * polarization angle used in the detector_response_equatorial functions
 *
 * Polarization angle is defined as:
 *
 *  tan \psi = ( J.z - (J.N)(z.N) ) / (N.(Jxz))
 *
 *  Where z is the axis of rotation of earth (equatorial z), and N is the line
 * of sight to the source (direction of propagation is -N)
 *
 *  The inclination angle is between the direction of propagation and the total
 * angular momentum
 */
template <class T>
void terr_pol_iota_from_equat_sph(
    T RA,     /**< Right ascension in rad*/
    T DEC,    /**< Declination in rad*/
    T thetaj, /**< spherical polar angle of the total angular momentum in
                 equatorial coordinates*/
    T phij,   /**< spherical azimuthal angle of the total angular momentum in
                 equatorial coordinates*/
    T* pol,   /**< polarization defined by tan \psi = ( J.z - (J.N)(z.N) ) /
                 (N.(Jxz)) */
    T* iota   /**< Inclination angle of the TOTAL angular momentum and the
                 direction of propagation -N*/
) {
  // std::cout<<"INTERNAL: RA: "<<RA<<" DEC: "<<DEC<<" thetaj: "<<thetaj<<"
  // phij: "<<phij<<std::endl; PSI only appears as 2*PSI in detector response,
  // so atan should be fine. Periodicity of pi
  T temp_pol = atan(cos(DEC) * 1. / tan(thetaj) * 1. / sin(phij - RA) -
                    1. / tan(phij - RA) * sin(DEC));
  *pol = M_PI / 2. - temp_pol;
  // if(*pol<0){*pol+=M_PI;}//Fix range
  //*iota = acos(cos(thetaj)*sin(DEC) + cos(DEC)*cos(phij - RA)*sin(thetaj));
  //-Neq
  *iota =
      acos(-(cos(thetaj) * sin(DEC)) - cos(DEC) * cos(phij - RA) * sin(thetaj));
}
template void terr_pol_iota_from_equat_sph<double>(double, double, double,
                                                   double, double*, double*);
template void terr_pol_iota_from_equat_sph<adouble>(adouble, adouble, adouble,
                                                    adouble, adouble*,
                                                    adouble*);
 ...
 *
 *
 * Rotation about the vernal equinox (x-hat in both coordinate systems) by the
 ...
 *
 * Needs the equatorial vectors for the line of sight and the spin angular
 * momentum
 *
 * Needs the inclination angle and the reference phase
 *
 * Works by constructing a third vector from the cross product of the two
 * others, in both frames. This defines a family of 3 vectors that can uniquely
 * determine a rotation matrix from source frame to equatorial, which was done
 * analytically in mathematica ( see the nb anglenb.nb)
 *
 * K = LxN
 *
 * A = {LSF,NSF, KSF}
 ...
 * R = B.A^-1
 *
 * Verified with specific cases and with dot products with vectors before and
 * after rotation
 */
template <class T>
void equatorial_from_SF(
    T* SFvec, /**< Input, source frame vector, as defined by LAL (L = z_hat) in
                 cartesian*/
    T thetal, /**< Polar angle in equatorial coordinates of the orbital angular
                 momentum at the reference frequency*/
    T phil,   /**< Azimuthal angle in equatorial coordinates of the orbital
                 angular momentum at the reference frequency*/
    T thetas, /**< Polar angle in equatorial coordinates of the source (not
                 direction of propagation)*/
    T phis,   /**< Azimuthal angle in equatorial coordinates of the source (not
                 direction of propagation)*/
    T iota, /**< Inclination angle between the orbiatl angular momentum and the
               direction of propagation at the reference frequency*/
    T phi_ref, /**<Reference frequency for the waveform (defines the LAL frame )
  ...
  // T EQvec_test[3];
  // T EQvec_old[3];
  // EQvec_old[0] =  (pow_int(cp,2)*cpl*Jzn*si*stl - ci*cpl*(cp*Jxn +
  // Jyn*sp)*stl + sp*(cpl*Jzn*si*sp*stl - cts*Jxn*spl*stl + cps*Jyn*sts +
  // ctl*Jxn*sps*sts) + cp*(cts*Jyn*spl*stl + cps*Jxn*sts -
  // ctl*Jyn*sps*sts))/(si*(pow_int(cp,2) + pow_int(sp,2))); EQvec_old[1] =
  // (pow_int(cp,2)*Jzn*si*spl*stl + cp*(-(cpl*cts*Jyn*stl) - ci*Jxn*spl*stl +
  // cps*ctl*Jyn*sts + Jxn*sps*sts) + sp*(cpl*cts*Jxn*stl - ci*Jyn*spl*stl +
  // Jzn*si*sp*spl*stl - cps*ctl*Jxn*sts + Jyn*sps*sts))/(si*(pow_int(cp,2) +
  // pow_int(sp,2))); EQvec_old[2] = (cp*cts*Jxn + pow_int(cp,2)*ctl*Jzn*si -
  // ci*ctl*(cp*Jxn + Jyn*sp) + cp*Jyn*(-(cps*spl) + cpl*sps)*stl*sts +
  // sp*(cts*Jyn + ctl*Jzn*si*sp + Jxn*(cps*spl -
  // cpl*sps)*stl*sts))/(si*(pow_int(cp,2) + pow_int(sp,2))); EQvec_test[0] =
  // (pow_int(cp,2)*cpl*Jzn*si*stl - ci*cpl*(cp*Jxn + Jyn*sp)*stl +
  // sp*(cpl*Jzn*si*sp*stl - cts*Jxn*spl*stl + cps*Jyn*sts + ctl*Jxn*sps*sts)
  // +cp*(cts*Jyn*spl*stl + cps*Jxn*sts - ctl*Jyn*sps*sts))/si ; EQvec_test[1] =
  // (pow_int(cp,2)*Jzn*si*spl*stl + cp*(-(cpl*cts*Jyn*stl) - ci*Jxn*spl*stl +
  // cps*ctl*Jyn*sts + Jxn*sps*sts) + sp*(cpl*cts*Jxn*stl - ci*Jyn*spl*stl +
  // Jzn*si*sp*spl*stl - cps*ctl*Jxn*sts + Jyn*sps*sts))/si; EQvec_test[2] =
  // (cp*cts*Jxn + pow_int(cp,2)*ctl*Jzn*si - ci*ctl*(cp*Jxn + Jyn*sp) +
  // cp*Jyn*(-(cps*spl) + cpl*sps)*stl*sts + sp*(cts*Jyn + ctl*Jzn*si*sp +
  // Jxn*(cps*spl - cpl*sps)*stl*sts))/si;

  EQvec[0] =
      (pow_int(cp, 2) * cpl * Jzn * si * stl -
       ci * cpl * (cp * Jxn + Jyn * sp) * stl -
       cp * (cts * Jyn * spl * stl + cps * Jxn * sts - ctl * Jyn * sps * sts) +
       sp * (cpl * Jzn * si * sp * stl + cts * Jxn * spl * stl -
             (cps * Jyn + ctl * Jxn * sps) * sts)) /
      si;
  EQvec[1] = (pow_int(cp, 2) * Jzn * si * spl * stl +
              sp * (-(cpl * cts * Jxn * stl) - ci * Jyn * spl * stl +
                    Jzn * si * sp * spl * stl + cps * ctl * Jxn * sts -
                    Jyn * sps * sts) +
              cp * (cpl * cts * Jyn * stl - ci * Jxn * spl * stl -
                    (cps * ctl * Jyn + Jxn * sps) * sts)) /
             si;
  EQvec[2] = (ctl * Jzn * si -
              Jxn * (ci * cp * ctl + cp * cts +
                     sp * (cps * spl - cpl * sps) * stl * sts) -
              Jyn * (ci * ctl * sp + cts * sp +
                     cp * (-(cps * spl) + cpl * sps) * stl * sts)) /
             si;
...
template void equatorial_from_SF<double>(double*, double, double, double,
                                         double, double, double, double*);
template void equatorial_from_SF<adouble>(adouble*, adouble, adouble, adouble,
                                          adouble, adouble, adouble, adouble*);
...
/*! \brief Calculates the larger mass given a chirp mass and symmetric mass
 * ratio
 *
...
double calculate_mass1(double chirpmass, double eta) {
  ...
  return 1. / 2 *
         (chirpmass / etapow + sqrt(1. - 4 * eta) * chirpmass / etapow);
}
...
/*! \brief Calculates the smaller mass given a chirp mass and symmetric mass
 * ratio
 *
 * Units of the output match the units of the input chirp mass
 */
...
/*! \brief Calculates the larger mass given a chirp mass and asymmetric mass
 * ratio
 *
 * Units of the output match the units of the input chirp mass
 */
...
/*! \brief Calculates the smaller mass given a chirp mass and asymmetric mass
 * ratio
 *
 * Units of the output match the units of the input chirp mass
 */
...
adouble pow_int(adouble base, int power) {
  ...
  adouble prod = 1;
  int pow = std::abs(power);
  for (int i = 0; i < pow; i++) prod = prod * base;
  if (power > 0)
    return prod;
  else
    return 1. / prod;
 ...
 */
double cbrt_internal(double base) { return cbrt(base); }
/*! \brief Fucntion that just returns the cuberoot
 * ADOL-C doesn't have the cbrt function (which is faster),
 * so have to use the power function
 */
adouble cbrt_internal(adouble base) { return pow(base, 1. / 3.); }
...
void tukey_window(double* window, int length, double alpha) {
  ...
  for (int i = 0; i < length; i++) {
    if (i < imin) {
      ...
      window[i] =
          0.5 *
          (1 + cos(M_PI * (((double)i) / ((double)(imin)) - 2. / alpha + 1.)));
      // window[i] = 0.5*(1 + cos(M_PI * ( (2. * i)/(alpha * length) - 2./alpha
      // + 1) ) ); window[i] = 0.5*(1 - cos(M_PI * ( (2. * i)/(alpha * length) )
      // ) );
    }
  }
}

/*! \brief Utility to transform from celestial coord RA and DEC to local horizon
 * coord for detector response functions
 *
 * Outputs are the spherical polar angles defined by North as 0 degrees azimuth
 * and the normal to the earth as 0 degree polar
 */
template <class T>
void celestial_horizon_transform(
    T RA,            /**< Right acsension (rad)*/
  ...
  T H = (LMST - (RA * 180. / M_PI) / 15.) * 15. * M_PI /
        180.;  // Local hour angle in rad

  T alt =
      asin(sin(DEC) * sin(LAT) + cos(DEC) * cos(LAT) * cos(H));  // alt in rad
  T a = acos((sin(DEC) - sin(alt) * sin(LAT)) /
             (cos(alt) * cos(LAT)));  // azimuth in rad
  T azimuth;
  if (sin(H) < 0)
    azimuth = a;
  else
...
template void celestial_horizon_transform<double>(double, double, double,
                                                  double, double, double*,
                                                  double*);
template void celestial_horizon_transform<adouble>(adouble, adouble, double,
...
T gps_to_GMST(T gps_time) {
  ...
  T GMST_unscaled = 6.697374558 + 0.06570982441908 * D0 + 1.00273790935 * H +
                    0.000026 * tau * tau;
  T hours;
  ...
  // return (6.697374558 + 0.06570982441908*D0 + 1.00273790935*H +
  // 0.000026*T*T);
...
void transform_cart_sph(T* cartvec, T* sphvec) {
  sphvec[0] = sqrt(cartvec[0] * cartvec[0] + cartvec[2] * cartvec[2] +
                   cartvec[1] * cartvec[1]);
...
std::complex<T> XLALSpinWeightedSphericalHarmonic(
  ...
  // if ( l < abs(s) )
  //{
  //   XLALPrintError("XLAL Error - %s: Invalid mode s=%d, l=%d, m=%d - require
  //   |s| <= l\n", __func__, s, l, m ); XLAL_ERROR_VAL(0, XLAL_EINVAL);
  // }
  // if ( l < abs(m) )
  //{
  //   XLALPrintError("XLAL Error - %s: Invalid mode s=%d, l=%d, m=%d - require
  ...
  if (s == -2) {
    if (l == 2) {
      switch (m) {
        case -2:
          fac = sqrt(5.0 / (64.0 * M_PI)) * (1.0 - cos(theta)) *
                (1.0 - cos(theta));
          break;
        case -1:
        ...
        case 2:
          ...
          //   require |m| <= l\n", __func__, s, l, m ); XLAL_ERROR_VAL(0,
          //   XLAL_EINVAL); break;
      } /*  switch (m) */
    } /* l==2*/
    else if (l == 3) {
      switch (m) {
        case -3:
          fac = sqrt(21.0 / (2.0 * M_PI)) * cos(theta / 2.0) *
                pow(sin(theta / 2.0), 5.0);
          break;
        case -2:
          fac = sqrt(7.0 / (4.0 * M_PI)) * (2.0 + 3.0 * cos(theta)) *
                pow(sin(theta / 2.0), 4.0);
          break;
        case -1:
          fac = sqrt(35.0 / (2.0 * M_PI)) *
                (sin(theta) + 4.0 * sin(2.0 * theta) - 3.0 * sin(3.0 * theta)) /
                32.0;
          break;
        case 0:
          fac =
              (sqrt(105.0 / (2.0 * M_PI)) * cos(theta) * pow(sin(theta), 2.0)) /
              4.0;
          break;
        case 1:
          fac = -sqrt(35.0 / (2.0 * M_PI)) *
        ...
        case 2:
          fac = sqrt(7.0 / M_PI) * pow(cos(theta / 2.0), 4.0) *
                (-2.0 + 3.0 * cos(theta)) / 2.0;
          break;

        case 3:
          fac = -sqrt(21.0 / (2.0 * M_PI)) * pow(cos(theta / 2.0), 5.0) *
                sin(theta / 2.0);
          break;
      ...
      switch (m) {
        case -4:
          fac = 3.0 * sqrt(7.0 / M_PI) * pow(cos(theta / 2.0), 2.0) *
                pow(sin(theta / 2.0), 6.0);
          break;
        case -3:
          fac = 3.0 * sqrt(7.0 / (2.0 * M_PI)) * cos(theta / 2.0) *
                (1.0 + 2.0 * cos(theta)) * pow(sin(theta / 2.0), 5.0);
          break;

        case -2:
          fac = (3.0 * (9.0 + 14.0 * cos(theta) + 7.0 * cos(2.0 * theta)) *
                 pow(sin(theta / 2.0), 4.0)) /
                (4.0 * sqrt(M_PI));
          break;
        case -1:
          fac = (3.0 * (3.0 * sin(theta) + 2.0 * sin(2.0 * theta) +
                        7.0 * sin(3.0 * theta) - 7.0 * sin(4.0 * theta))) /
                (32.0 * sqrt(2.0 * M_PI));
          break;
        case 0:
          fac = (3.0 * sqrt(5.0 / (2.0 * M_PI)) *
                 (5.0 + 7.0 * cos(2.0 * theta)) * pow(sin(theta), 2.0)) /
                16.0;
          break;
        case 1:
          fac = (3.0 * (3.0 * sin(theta) - 2.0 * sin(2.0 * theta) +
          ...
          break;
        case 2:
          fac = (3.0 * pow(cos(theta / 2.0), 4.0) *
                 (9.0 - 14.0 * cos(theta) + 7.0 * cos(2.0 * theta))) /
                (4.0 * sqrt(M_PI));
          break;
        case 3:
          fac = -3.0 * sqrt(7.0 / (2.0 * M_PI)) * pow(cos(theta / 2.0), 5.0) *
                (-1.0 + 2.0 * cos(theta)) * sin(theta / 2.0);
          break;
        case 4:
          fac = 3.0 * sqrt(7.0 / M_PI) * pow(cos(theta / 2.0), 6.0) *
      ...
      switch (m) {
        case -5:
          fac = sqrt(330.0 / M_PI) * pow(cos(theta / 2.0), 3.0) *
                pow(sin(theta / 2.0), 7.0);
          break;
        case -4:
          fac = sqrt(33.0 / M_PI) * pow(cos(theta / 2.0), 2.0) *
                (2.0 + 5.0 * cos(theta)) * pow(sin(theta / 2.0), 6.0);
          break;
        case -3:
          fac = (sqrt(33.0 / (2.0 * M_PI)) * cos(theta / 2.0) *
                 (17.0 + 24.0 * cos(theta) + 15.0 * cos(2.0 * theta)) *
                 pow(sin(theta / 2.0), 5.0)) /
                4.0;
          break;
        case -2:
          fac = (sqrt(11.0 / M_PI) *
                 (32.0 + 57.0 * cos(theta) + 36.0 * cos(2.0 * theta) +
                  15.0 * cos(3.0 * theta)) *
                 pow(sin(theta / 2.0), 4.0)) /
                8.0;
          break;
        case -1:
          fac = (sqrt(77.0 / M_PI) *
                 (2.0 * sin(theta) + 8.0 * sin(2.0 * theta) +
                  3.0 * sin(3.0 * theta) + 12.0 * sin(4.0 * theta) -
                  15.0 * sin(5.0 * theta))) /
                256.0;
          break;
        case 0:
          fac = (sqrt(1155.0 / (2.0 * M_PI)) *
                 (5.0 * cos(theta) + 3.0 * cos(3.0 * theta)) *
                 pow(sin(theta), 2.0)) /
                32.0;
          break;
        case 1:
          ...
          break;
        case 2:
          fac = sqrt(11.0 / M_PI) * pow(cos(theta / 2.0), 4.0) *
                (-32.0 + 57.0 * cos(theta) - 36.0 * cos(2.0 * theta) +
                 15.0 * cos(3.0 * theta)) /
                8.0;
          break;
        case 3:
          fac = -sqrt(33.0 / (2.0 * M_PI)) * pow(cos(theta / 2.0), 5.0) *
                (17.0 - 24.0 * cos(theta) + 15.0 * cos(2.0 * theta)) *
                sin(theta / 2.0) / 4.0;
          break;
        case 4:
          fac = sqrt(33.0 / M_PI) * pow(cos(theta / 2.0), 6.0) *
                (-2.0 + 5.0 * cos(theta)) * pow(sin(theta / 2.0), 2.0);
          break;
        case 5:
          fac = -sqrt(330.0 / M_PI) * pow(cos(theta / 2.0), 7.0) *
                pow(sin(theta / 2.0), 3.0);
          break;
          // default:
          //   XLALPrintError("XLAL Error - %s: Invalid mode s=%d, l=%d, m=%d -
          //   require |m| <= l\n", __func__, s, l, m ); XLAL_ERROR_VAL(0,
      ...
      switch (m) {
        case -6:
          fac = (3. * sqrt(715. / M_PI) * pow(cos(theta / 2.0), 4) *
                 pow(sin(theta / 2.0), 8)) /
                2.0;
          break;
        case -5:
          fac = (sqrt(2145. / M_PI) * pow(cos(theta / 2.0), 3) *
                 (1. + 3. * cos(theta)) * pow(sin(theta / 2.0), 7)) /
                2.0;
          break;
        case -4:
          fac = (sqrt(195. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 2) *
                 (35. + 44. * cos(theta) + 33. * cos(2. * theta)) *
                 pow(sin(theta / 2.0), 6)) /
                8.0;
          break;
        case -3:
          fac = (3. * sqrt(13. / M_PI) * cos(theta / 2.0) *
                 (98. + 185. * cos(theta) + 110. * cos(2 * theta) +
                  55. * cos(3. * theta)) *
                 pow(sin(theta / 2.0), 5)) /
                32.0;
          break;
        case -2:
          fac = (sqrt(13. / M_PI) *
                 (1709. + 3096. * cos(theta) + 2340. * cos(2. * theta) +
                  1320. * cos(3. * theta) + 495. * cos(4. * theta)) *
                 pow(sin(theta / 2.0), 4)) /
                256.0;
          break;
        case -1:
          fac = (sqrt(65. / (2.0 * M_PI)) * cos(theta / 2.0) *
                 (161. + 252. * cos(theta) + 252. * cos(2. * theta) +
                  132. * cos(3. * theta) + 99. * cos(4. * theta)) *
                 pow(sin(theta / 2.0), 3)) /
                64.0;
          break;
        case 0:
          fac = (sqrt(1365. / M_PI) *
                 (35. + 60. * cos(2. * theta) + 33. * cos(4. * theta)) *
                 pow(sin(theta), 2)) /
                512.0;
          break;
        case 1:
          fac = (sqrt(65. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 3) *
                 (161. - 252. * cos(theta) + 252. * cos(2. * theta) -
                  132. * cos(3. * theta) + 99. * cos(4. * theta)) *
                 sin(theta / 2.0)) /
                64.0;
          break;
        case 2:
          fac = (sqrt(13. / M_PI) * pow(cos(theta / 2.0), 4) *
                 (1709. - 3096. * cos(theta) + 2340. * cos(2. * theta) -
                  1320 * cos(3 * theta) + 495 * cos(4 * theta))) /
                256.0;
          break;
        case 3:
          fac = (-3. * sqrt(13. / M_PI) * pow(cos(theta / 2.0), 5) *
                 (-98. + 185. * cos(theta) - 110. * cos(2 * theta) +
                  55. * cos(3. * theta)) *
                 sin(theta / 2.0)) /
                32.0;
          break;
        case 4:
          fac = (sqrt(195. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 6) *
                 (35. - 44. * cos(theta) + 33. * cos(2 * theta)) *
                 pow(sin(theta / 2.0), 2)) /
                8.0;
          break;
        case 5:
          fac = -(sqrt(2145. / M_PI) * pow(cos(theta / 2.0), 7) *
                  (-1. + 3. * cos(theta)) * pow(sin(theta / 2.0), 3)) /
                2.0;
          break;
        case 6:
          fac = (3. * sqrt(715. / M_PI) * pow(cos(theta / 2.0), 8) *
                 pow(sin(theta / 2.0), 4)) /
                2.0;
          break;
          // default:
      ...
      switch (m) {
        case -7:
          fac = sqrt(15015. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 5) *
                pow(sin(theta / 2.0), 9);
          break;
        case -6:
          fac = (sqrt(2145. / M_PI) * pow(cos(theta / 2.0), 4) *
                 (2. + 7. * cos(theta)) * pow(sin(theta / 2.0), 8)) /
                2.0;
          break;
        case -5:
          fac = (sqrt(165. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 3) *
                 (93. + 104. * cos(theta) + 91. * cos(2. * theta)) *
                 pow(sin(theta / 2.0), 7)) /
                8.0;
          break;
        case -4:
          fac = (sqrt(165. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 2) *
                 (140. + 285. * cos(theta) + 156. * cos(2. * theta) +
                  91. * cos(3. * theta)) *
                 pow(sin(theta / 2.0), 6)) /
                16.0;
          break;
        case -3:
          fac = (sqrt(15. / (2.0 * M_PI)) * cos(theta / 2.0) *
                 (3115. + 5456. * cos(theta) + 4268. * cos(2. * theta) +
                  2288. * cos(3. * theta) + 1001. * cos(4. * theta)) *
                 pow(sin(theta / 2.0), 5)) /
                128.0;
          break;
        case -2:
          fac = (sqrt(15. / M_PI) *
                 (5220. + 9810. * cos(theta) + 7920. * cos(2. * theta) +
                  5445. * cos(3. * theta) + 2860. * cos(4. * theta) +
                  1001. * cos(5. * theta)) *
                 pow(sin(theta / 2.0), 4)) /
                512.0;
          break;
        case -1:
          fac = (3. * sqrt(5. / (2.0 * M_PI)) * cos(theta / 2.0) *
                 (1890. + 4130. * cos(theta) + 3080. * cos(2. * theta) +
                  2805. * cos(3. * theta) + 1430. * cos(4. * theta) +
                  1001. * cos(5 * theta)) *
                 pow(sin(theta / 2.0), 3)) /
                512.0;
          break;
        case 0:
          fac = (3. * sqrt(35. / M_PI) * cos(theta) *
                 (109. + 132. * cos(2. * theta) + 143. * cos(4. * theta)) *
                 pow(sin(theta), 2)) /
                512.0;
          break;
        case 1:
          fac = (3. * sqrt(5. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 3) *
                 (-1890. + 4130. * cos(theta) - 3080. * cos(2. * theta) +
                  2805. * cos(3. * theta) - 1430. * cos(4. * theta) +
                  1001. * cos(5. * theta)) *
                 sin(theta / 2.0)) /
          ...
          break;
        case 2:
          fac = (sqrt(15. / M_PI) * pow(cos(theta / 2.0), 4) *
                 (-5220. + 9810. * cos(theta) - 7920. * cos(2. * theta) +
                  5445. * cos(3. * theta) - 2860. * cos(4. * theta) +
                  1001. * cos(5. * theta))) /
                512.0;
          break;
        case 3:
          fac = -(sqrt(15. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 5) *
                  (3115. - 5456. * cos(theta) + 4268. * cos(2. * theta) -
                   2288. * cos(3. * theta) + 1001. * cos(4. * theta)) *
                  sin(theta / 2.0)) /
                128.0;
          break;
        case 4:
          fac = (sqrt(165. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 6) *
                 (-140. + 285. * cos(theta) - 156. * cos(2 * theta) +
                  91. * cos(3. * theta)) *
                 pow(sin(theta / 2.0), 2)) /
                16.0;
          break;
        case 5:
          fac = -(sqrt(165. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 7) *
                  (93. - 104. * cos(theta) + 91. * cos(2. * theta)) *
                  pow(sin(theta / 2.0), 3)) /
                8.0;
          break;
        case 6:
          fac = (sqrt(2145. / M_PI) * pow(cos(theta / 2.0), 8) *
                 (-2. + 7. * cos(theta)) * pow(sin(theta / 2.0), 4)) /
                2.0;
          break;
        case 7:
          fac = -(sqrt(15015. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 9) *
                  pow(sin(theta / 2.0), 5));
          break;
          // default:
          //   XLALPrintError("XLAL Error - %s: Invalid mode s=%d, l=%d, m=%d -
      ...
      switch (m) {
        case -8:
          fac = sqrt(34034. / M_PI) * pow(cos(theta / 2.0), 6) *
                pow(sin(theta / 2.0), 10);
          break;
        case -7:
          fac = sqrt(17017. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 5) *
                (1. + 4. * cos(theta)) * pow(sin(theta / 2.0), 9);
          break;
        case -6:
          fac = sqrt(255255. / M_PI) * pow(cos(theta / 2.0), 4) *
                (1. + 2. * cos(theta)) * sin(M_PI / 4.0 - theta / 2.0) *
                sin(M_PI / 4.0 + theta / 2.0) * pow(sin(theta / 2.0), 8);
          break;
        case -5:
          fac = (sqrt(12155. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 3) *
                 (19. + 42. * cos(theta) + 21. * cos(2. * theta) +
                  14. * cos(3. * theta)) *
                 pow(sin(theta / 2.0), 7)) /
                8.0;
          break;
        case -4:
          fac = (sqrt(935. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 2) *
                 (265. + 442. * cos(theta) + 364. * cos(2. * theta) +
                  182. * cos(3. * theta) + 91. * cos(4. * theta)) *
                 pow(sin(theta / 2.0), 6)) /
                32.0;
          break;
        case -3:
          fac = (sqrt(561. / (2.0 * M_PI)) * cos(theta / 2.0) *
                 (869. + 1660. * cos(theta) + 1300. * cos(2. * theta) +
                  910. * cos(3. * theta) + 455. * cos(4. * theta) +
                  182. * cos(5. * theta)) *
                 pow(sin(theta / 2.0), 5)) /
                128.0;
          break;
        case -2:
          fac = (sqrt(17. / M_PI) *
                 (7626. + 14454. * cos(theta) + 12375. * cos(2. * theta) +
                  9295. * cos(3. * theta) + 6006. * cos(4. * theta) +
                  3003. * cos(5. * theta) + 1001. * cos(6. * theta)) *
                 pow(sin(theta / 2.0), 4)) /
                512.0;
          break;
        case -1:
          fac = (sqrt(595. / (2.0 * M_PI)) * cos(theta / 2.0) *
                 (798. + 1386. * cos(theta) + 1386. * cos(2. * theta) +
                  1001. * cos(3. * theta) + 858. * cos(4. * theta) +
                  429. * cos(5. * theta) + 286. * cos(6. * theta)) *
                 pow(sin(theta / 2.0), 3)) /
                512.0;
          break;
        case 0:
          fac = (3. * sqrt(595. / M_PI) *
                 (210. + 385. * cos(2. * theta) + 286. * cos(4. * theta) +
                  143. * cos(6. * theta)) *
                 pow(sin(theta), 2)) /
                4096.0;
          break;
        case 1:
          fac = (sqrt(595. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 3) *
                 (798. - 1386. * cos(theta) + 1386. * cos(2. * theta) -
                  1001. * cos(3. * theta) + 858. * cos(4. * theta) -
                  429. * cos(5. * theta) + 286. * cos(6. * theta)) *
                 sin(theta / 2.0)) /
          ...
          break;
        case 2:
          fac = (sqrt(17. / M_PI) * pow(cos(theta / 2.0), 4) *
                 (7626. - 14454. * cos(theta) + 12375. * cos(2. * theta) -
                  9295. * cos(3. * theta) + 6006. * cos(4. * theta) -
                  3003. * cos(5. * theta) + 1001. * cos(6. * theta))) /
                512.0;
          break;
        case 3:
          fac = -(sqrt(561. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 5) *
                  (-869. + 1660. * cos(theta) - 1300. * cos(2. * theta) +
                   910. * cos(3. * theta) - 455. * cos(4. * theta) +
                   182. * cos(5. * theta)) *
                  sin(theta / 2.0)) /
          ...
          break;
        case 4:
          fac = (sqrt(935. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 6) *
                 (265. - 442. * cos(theta) + 364. * cos(2. * theta) -
                  182. * cos(3. * theta) + 91. * cos(4. * theta)) *
                 pow(sin(theta / 2.0), 2)) /
                32.0;
          break;
        case 5:
          fac = -(sqrt(12155. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 7) *
                  (-19. + 42. * cos(theta) - 21. * cos(2. * theta) +
                   14. * cos(3. * theta)) *
                  pow(sin(theta / 2.0), 3)) /
                8.0;
          break;
        case 6:
          fac = sqrt(255255. / M_PI) * pow(cos(theta / 2.0), 8) *
                (-1. + 2. * cos(theta)) * sin(M_PI / 4.0 - theta / 2.0) *
                sin(M_PI / 4.0 + theta / 2.0) * pow(sin(theta / 2.0), 4);
          break;
        case 7:
          fac = -(sqrt(17017. / (2.0 * M_PI)) * pow(cos(theta / 2.0), 9) *
                  (-1. + 4. * cos(theta)) * pow(sin(theta / 2.0), 5));
          break;
        case 8:
          fac = sqrt(34034. / M_PI) * pow(cos(theta / 2.0), 10) *
                pow(sin(theta / 2.0), 6);
          break;
          // default:
          //   XLALPrintError("XLAL Error - %s: Invalid mode s=%d, l=%d, m=%d -
    ...
    // else
    //{
    //   XLALPrintError("XLAL Error - %s: Unsupported mode l=%d (only l in [2,8]
    //   implemented)\n", __func__, l); XLAL_ERROR_VAL(0, XLAL_EINVAL);
    // }
  }
  // else
  //{
  //   XLALPrintError("XLAL Error - %s: Unsupported mode s=%d (only s=-2
  //   implemented)\n", __func__, s); XLAL_ERROR_VAL(0, XLAL_EINVAL);
  // }
  if (m)
...
template std::complex<double> XLALSpinWeightedSphericalHarmonic<double>(
    double, double, int, int, int);
template std::complex<adouble> XLALSpinWeightedSphericalHarmonic<adouble>(
    adouble, adouble, int, int, int);
