...
/*!\file
 *File that handles the construction of the (2,2) waveform as described by
 * IMRPhenomD by Khan et. al.
 *
...
template <class T>
int time_waveform(
    T* times, /**< double array of frequencies for the waveform to be evaluated
                 at*/
    int length,                    /**<integer length of all the arrays*/
    waveform_polarizations<T>* wp, /**< [out] Output waveforms by polarization*/
    string generation_method,      /**<String that corresponds to the generation
                                      method - MUST BE SPELLED EXACTLY*/
    gen_params_base<T>*
  ...
  std::string local_method =
      prep_source_parameters(&params, parameters, generation_method);
  if (has_substring(local_method, "Taylor")) {
    // std::complex<T> ci = std::complex<T>(cos(params.incl_angle),0);
    if (local_method == "TaylorT2") {
      TaylorT2<T> T2model;
      status = T2model.construct_waveform(times, length, wp->hplus, wp->hcross,
                                          &params);
    }
  }
...
/*!\brief Function to produce the plus/cross polarizations of an quasi-circular
 * binary
 *
 * By using the structure parameter, the function is allowed to be more flexible
 * in using different method of waveform generation - not all methods use the
 * same parameters
 *
 ...
 *
 * If Phic is unassigned, a reference phase AND a reference frequency are looked
 * for.If no options are found, both are set to 0.
 *
 ...
 *
 * tc, if specified, is used with the use of interpolation. If not, tc is set
 * such that coalescence happens at t=0
 */
template <class T>
int fourier_waveform(
    T* frequencies, /**< double array of frequencies for the waveform to be
                       evaluated at*/
    int length,     /**<integer length of all the arrays*/
    waveform_polarizations<T>* wp, /**< [out] Output waveforms by polarization*/
    // std::complex<T> *waveform_plus, /**< [out] complex array for the output
    // plus polarization waveform*/ std::complex<T> *waveform_cross, /**< [out]
    // complex array for the output cross polarization waveform*/
    string generation_method, /**<String that corresponds to the generation
                                 method - MUST BE SPELLED EXACTLY*/
    gen_params_base<T>*
        parameters /**<structure containing all the source parameters*/
   ...
   * quantities once*/
  source_parameters<T> params;
  if (generation_method == "IMRPhenomD_NRT_EOS") {
    IMRPhenomD_NRT_EOS<T>::get_m_love(parameters);
  }
  ...
  if (has_substring(local_method, "IMRPhenomD")) {
    ...
    if (local_method == "ppE_IMRPhenomD_Inspiral") {
      ...
      status =
          ppemodeld.construct_waveform(frequencies, length, wp->hplus, &params);
      ...
      status =
          gmodeld.construct_waveform(frequencies, length, wp->hplus, &params);

    } else if (local_method == "IMRPhenomD_NRT") {
      IMRPhenomD_NRT<T> modeldNRT;
      status =
          modeldNRT.construct_waveform(frequencies, length, wp->hplus, &params);

    } else if (local_method == "IMRPhenomD_NRT_EOS") {
      IMRPhenomD_NRT_EOS<T> modelNRTEOS;
      status = modelNRTEOS.construct_waveform(frequencies, length, wp->hplus,
                                               &params);
      ...
      status =
          EAmodeldNRT.EA_construct_waveform(frequencies, length, wp, &params);
      ...
      IMRPhenomD<T> modeld;
      status =
          modeld.construct_waveform(frequencies, length, wp->hplus, &params);
    }
    if (restrictedEval) {
      // for (int i =0 ; i < length; i++){
      //	wp->hcross[i] = ci*std::complex<T>(0,-1) * wp->hplus[i];
      //	wp->hplus[i] = wp->hplus[i]* std::complex<T>(.5,0)
      //*(std::complex<T>(1,0)+ci*ci);
      // }
      for (int i = 0; i < length; i++) {
    ...
    /*! Iota is weird, because it's a extrinsic parameter, but one that has
     * dependence on frequency (potentially)*/
    /*! I think it should be handled separately because of complications like
     * LISA response, etc*/
    for (int i = 0; i < length; i++) {
      wp->hcross[i] *= ci;
      wp->hplus[i] *=
          std::complex<T>(.5, 0) * (std::complex<T>(1, 0) + ci * ci);
    }
    ...
    if (local_method == "ppE_IMRPhenomPv2_Inspiral") {
      ...
      status = ppemodel.construct_waveform(frequencies, length, wp->hplus,
                                           wp->hcross, &params);
      ...
      status = gmodel.construct_waveform(frequencies, length, wp->hplus,
                                         wp->hcross, &params);

    } else if (local_method == "IMRPhenomPv2_NRT") {
      IMRPhenomPv2_NRT<T> modelNRT;
      status = modelNRT.construct_waveform(frequencies, length, wp->hplus,
                                           wp->hcross, &params);

    } else {
      IMRPhenomPv2<T> model;
      status = model.construct_waveform(frequencies, length, wp->hplus,
                                        wp->hcross, &params);
    }
    ...
    if (has_substring(local_method, "fast")) {
      if (has_substring(local_method, "uniform"))
        status = efpe_fast_fourier_waveform_uniform(frequencies, length, wp,
                                                    parameters);
      else
        status =
            efpe_fast_fourier_waveform(frequencies, length, wp, parameters);
    } else {
      if (has_substring(local_method, "uniform"))
        status =
            efpe_fourier_waveform_uniform(frequencies, length, wp, parameters);
      else
        status = efpe_fourier_waveform(frequencies, length, wp, parameters);
    }
  }

  // Catch all for any modifications not captured in ppE formalism like extra
  // polarizations
  extra_modifications(generation_method, parameters, &params, wp, frequencies,
                      length);
...
int fourier_waveform(
    double* frequencies, /**< double array of frequencies for the waveform to be
                            evaluated at*/
    int length,          /**<integer length of all the arrays*/
  ...
  // std::complex<double> *waveform_plus = (std::complex<double>
  // *)malloc(sizeof(std::complex<double>)*length); std::complex<double>
  // *waveform_cross = (std::complex<double>
  // *)malloc(sizeof(std::complex<double>)*length);
  ...
  wp->hb = waveform_b;
  wp->hl = waveform_l;
  // int status = fourier_waveform(frequencies, length,
  // waveform_plus,waveform_cross, generation_method, parameters);
  int status =
      fourier_waveform(frequencies, length, wp, generation_method, parameters);
...
int fourier_waveform(
  ...
  if (generation_method == "IMRPhenomD_NRT_EOS") {
    IMRPhenomD_NRT_EOS<double>::get_m_love(parameters);
  }
  ...
  // params = params.populate_source_parameters(mass1, mass2,
  // Luminosity_Distance, spin1, spin2, phi_c,t_c); params =
  ...
  if (generation_method == "IMRPhenomD") {
    ...
    status =
        ppemodeld.construct_waveform(frequencies, length, waveform, &params);
  }
  ...
  //	status = ppemodeld.construct_waveform(frequencies, length, waveform,
  //&params); 	for( int i = 0; i < params.Nmod; i++)
  ...
  // }
    ...
    status =
        modeldNRT.construct_waveform(frequencies, length, waveform, &params);

  } else if (generation_method == "IMRPhenomD_NRT_EOS") {
    IMRPhenomD_NRT_EOS<double> modelNRTEOS;
    status =
        modelNRTEOS.construct_waveform(frequencies, length, waveform, &params);
  }
  // else if(generation_method == "IMRPhenomPv2")
  //{
  //	IMRPhenomPv2<double> modeld;
  //	//Initialize Pv2 specific params
  //	status = modeld.construct_waveform(frequencies, length, waveform,
  //&params);
  // }
...
int fourier_waveform(
  ...
  int status = fourier_waveform(frequencies, length, waveform,
...
/*!\brief Function to produce the amplitude of the (2,2) mode of an
...
int fourier_amplitude(
  ...
  if (has_substring(local_method, "IMRPhenomD")) {
    ...
    if (local_method == "ppE_IMRPhenomD_Inspiral") {
      ppE_IMRPhenomD_Inspiral<T> ppemodeld;
      status = ppemodeld.construct_amplitude(frequencies, length, amplitude,
                                             &params);
      ...
      gIMRPhenomD<T> gmodeld;
      status =
          gmodeld.construct_amplitude(frequencies, length, amplitude, &params);

    } else if (local_method == "IMRPhenomD_NRT") {
      IMRPhenomD_NRT<T> modeldNRT;
      status = modeldNRT.construct_amplitude(frequencies, length, amplitude,
                                             &params);

    } else if (local_method == "IMRPhenomD_NRT_EOS") {
      IMRPhenomD_NRT_EOS<T> modelNRTEOS;
      status = modelNRTEOS.construct_amplitude(frequencies, length, amplitude,
                                                &params);

    } else if (local_method == "EA_IMRPhenomD_NRT") {
      EA_IMRPhenomD_NRT<T> EAmodeldNRT;
      status = EAmodeldNRT.construct_amplitude(frequencies, length, amplitude,
                                               &params);

    } else {
      IMRPhenomD<T> modeld;
      status =
          modeld.construct_amplitude(frequencies, length, amplitude, &params);
    }
  }
  cleanup_source_parameters(&params, generation_method);
  ...
  //{
  //	IMRPhenomD<T> modeld;
  //	status = modeld.construct_amplitude(frequencies, length, amplitude,
  //&params);
  // }
  // else if(generation_method == "ppE_IMRPhenomD_Inspiral")
  //{
  ...
  //	status = gmodeld.construct_amplitude(frequencies, length, amplitude,
  //&params);
  // }

  return status;
}
template int fourier_amplitude<double>(double*, int, double*, std::string,
                                       gen_params_base<double>*);
template int fourier_amplitude<adouble>(adouble*, int, adouble*, std::string,
                                        gen_params_base<adouble>*);
/*!\brief Function to produce the phase of the (2,2) mode of an quasi-circular
 * binary
 *
 * By using the structure parameter, the function is allowed to be more flexible
 * in using different method of waveform generation - not all methods use the
...
int fourier_phase(
    ...
    string generation_method, /**<String that corresponds to the generation
                                 method - MUST BE SPELLED EXACTLY*/
    gen_params_base<T>* parameters) {
  std::cout << "Possible error in negative sign! Deprecated, please use the "
               "version with (+) and (x) separate"
            << std::endl;
  int status = 1;
  bool NSflag1 = parameters->NSflag1;
  bool NSflag2 = parameters->NSflag2;
  ...
  // }
    ...
    params.tidal1 = parameters->tidal1;
    params.tidal2 = parameters->tidal2;
    status = modelNRTEOS.construct_phase(frequencies, length, phase, &params);
  } else if (local_method == "EA_IMRPhenomD_NRT") {
    EA_IMRPhenomD_NRT<T> EAmodeldNRT;
    params.tidal1 = parameters->tidal1;
    params.tidal2 = parameters->tidal2;
...
template int fourier_phase<double>(double*, int, double*, std::string,
                                   gen_params_base<double>*);
template int fourier_phase<adouble>(adouble*, int, adouble*, std::string,
                                    gen_params_base<adouble>*);
/*!\brief Function to produce the phase of the plus and cross mode of a
 * quasi-circular binary
 *
 * By using the structure parameter, the function is allowed to be more flexible
 * in using different method of waveform generation - not all methods use the
...
int fourier_phase(
  ...
  if (has_substring(local_method, "IMRPhenomD")) {
    if (local_method == "ppE_IMRPhenomD_Inspiral") {
      ppE_IMRPhenomD_Inspiral<T> ppemodeld;
      status =
          ppemodeld.construct_phase(frequencies, length, phase_plus, &params);
      ...
      gIMRPhenomD<T> gmodeld;
      status =
          gmodeld.construct_phase(frequencies, length, phase_plus, &params);

    } else if (local_method == "IMRPhenomD_NRT") {
      IMRPhenomD_NRT<T> modeldNRT;
      status =
          modeldNRT.construct_phase(frequencies, length, phase_plus, &params);

    } else if (local_method == "IMRPhenomD_NRT_EOS") {
      IMRPhenomD_NRT_EOS<T> modelNRTEOS;
      status = modelNRTEOS.construct_phase(frequencies, length, phase_plus,
                                            &params);

    } else if (local_method == "EA_IMRPhenomD_NRT") {
      EA_IMRPhenomD_NRT<T> EAmodeldNRT;
      status =
          EAmodeldNRT.construct_phase(frequencies, length, phase_plus, &params);
    ...
    if (local_method == "ppE_IMRPhenomPv2_Inspiral") {
      ppE_IMRPhenomPv2_Inspiral<T> ppemodel;
      status = ppemodel.construct_phase(frequencies, length, phase_plus_temp,
                                        phase_cross_temp, &params);
      ...
      gIMRPhenomPv2<T> gmodel;
      status = gmodel.construct_phase(frequencies, length, phase_plus_temp,
                                      phase_cross_temp, &params);

    } else if (local_method == "IMRPhenomPv2_NRT") {
      IMRPhenomPv2_NRT<T> modelNRT;
      status = modelNRT.construct_phase(frequencies, length, phase_plus_temp,
                                        phase_cross_temp, &params);

    } else {
      IMRPhenomPv2<T> model;
      status = model.construct_phase(frequencies, length, phase_plus_temp,
                                     phase_cross_temp, &params);
    }
  ...
  //{
  //	IMRPhenomD<T> modeld;
  //	status = modeld.construct_phase(frequencies, length, phase_plus,
  //&params); 	for(int i = 0 ; i<length; i++){ 		phase_cross[i] =
  // phase_plus[i]+ M_PI/2.;
  //		//phase_plus[i]*= (T)(-1.);
  //		//phase_cross[i] = phase_plus[i]- M_PI/2.;
  //	}
  ...
  //	params.Nmod = parameters->Nmod;
  //	ppE_IMRPhenomD_Inspiral<T> ppemodeld;
  //	status = ppemodeld.construct_phase(frequencies, length, phase_plus,
  //&params); 	for(int i = 0 ; i<length; i++){
  //		//phase_plus[i]*= (T)(-1.);
  //		phase_cross[i] = phase_plus[i]+ M_PI/2.;
  //	}
  // }
  ...
  //	params.Nmod_alpha = parameters->Nmod_alpha;
  //	gIMRPhenomD<T> gmodeld;
  //	status = gmodeld.construct_phase(frequencies, length, phase_plus,
  //&params); 	for(int i = 0 ; i<length; i++){
  //		//phase_plus[i]*= (T)(-1.);
  //		phase_cross[i] = phase_plus[i]+ M_PI/2.;
  //	}
  ...
  //	T *phase_plus_temp = new T[length];
  //	T *phase_cross_temp = new T[length];
  //	status = gmodelp.construct_phase(frequencies, length, phase_plus_temp,
  // phase_cross_temp, &params); 	unwrap_array(phase_plus_temp,
  // phase_plus, length); 	unwrap_array(phase_cross_temp, phase_cross,
  // length); 	delete [] phase_plus_temp; 	delete [] phase_cross_temp;
  //}
  // else if(generation_method == "IMRPhenomPv2")
  //{
  //	//std::complex<T> ci = std::complex<T>(cos(params.incl_angle),0);
  ...
  //	T *phase_plus_temp = new T[length];
  //	T *phase_cross_temp = new T[length];
  //	status = modeld.construct_phase(frequencies, length, phase_plus_temp,
  // phase_cross_temp, &params); 	unwrap_array(phase_plus_temp,
  // phase_plus, length); 	unwrap_array(phase_cross_temp, phase_cross,
  // length); 	delete [] phase_plus_temp; 	delete [] phase_cross_temp;
  //	//for(int i = 0 ; i<length; i++){
...
// template int fourier_waveform<double>(double *, int, std::complex<double>
// *,std::complex<double> *, std::string, gen_params_base<double> *); template
// int fourier_waveform<adouble>(adouble *, int, std::complex<adouble>
// *,std::complex<adouble> *, std::string, gen_params_base<adouble> *);
template int fourier_waveform<double>(double*, int,
                                      waveform_polarizations<double>* wp,
                                      std::string, gen_params_base<double>*);
template int fourier_waveform<adouble>(adouble*, int,
                                       waveform_polarizations<adouble>* wp,
                                       std::string, gen_params_base<adouble>*);

template int fourier_phase<double>(double*, int, double*, double*, std::string,
                                   gen_params_base<double>*);
template int fourier_phase<adouble>(adouble*, int, adouble*, adouble*,
                                    std::string, gen_params_base<adouble>*);

template int time_waveform<double>(double*, int,
                                   waveform_polarizations<double>* wp,
                                   std::string, gen_params_base<double>*);
template int time_waveform<adouble>(adouble*, int,
                                    waveform_polarizations<adouble>* wp,
                                    std::string, gen_params_base<adouble>*);

template <class T>
std::string prep_source_parameters(source_parameters<T>* out,
                                   gen_params_base<T>* in,
  ...
  if (has_substring(generation_method, "Pv2")) {
    ...
    if (in->mass1 < in->mass2) {
      PhenomPrecessingSpinEnforcePrimary(
          &(in->mass1), &(in->mass2), &(in->spin1[0]), &(in->spin1[1]),
          &(in->spin1[2]), &(in->spin2[0]), &(in->spin2[1]), &(in->spin2[2]));
      out->populate_source_parameters(in);
    }
  ...
  if (has_substring(generation_method, "NRT")) {
    ...
    if ((in->tidal1 < 0 || in->tidal2 < 0) && in->tidal_weighted >= 0) {
      ...
      // arXiv 1402.5156, Eqs (5) (tidal_weighted) and (6)
      // (delta_tidal_weighted)
      out->tidal_weighted =
          8. / 13. *
          ((1. + 7. * out->eta - 31. * out->eta * out->eta) *
               (out->tidal1 + out->tidal2) +
           sqrt(1. - 4. * out->eta) *
               (1. + 9. * out->eta - 11. * out->eta * out->eta) *
               (out->tidal1 - out->tidal2));

      out->delta_tidal_weighted =
          1. / 2. *
          (sqrt(1. - 4. * out->eta) *
               (1. - 13272. / 1319. * out->eta +
                8944. / 1319. * out->eta * out->eta) *
               (out->tidal1 + out->tidal2) +
           (1. - 15910. / 1319. * out->eta +
            32850. / 1319. * out->eta * out->eta +
            3380. / 1319. * out->eta * out->eta * out->eta) *
               (out->tidal1 - out->tidal2));
      // debugger_print(__FILE__,__LINE__,out->tidal_weighted);
    }
    // Dissipative tidal deformability
    if ((in->diss_tidal1 < 0 || in->diss_tidal2 < 0) &&
        in->diss_tidal_weighted >= 0) {
      ...
      out->diss_tidal_weighted =
          (2.0 * pow(out->eta, 2) - 4.0 * out->eta + 1.0) * 0.5 *
              (out->diss_tidal1 + out->diss_tidal2) -
          sqrt(1.0 - 4.0 * out->eta) * (1.0 - 2.0 * out->eta) * 0.5 *
              (out->diss_tidal1 - out->diss_tidal2);
    }
  }
...
template std::string prep_source_parameters(source_parameters<adouble>*,
                                            gen_params_base<adouble>*,
                                            std::string);
template std::string prep_source_parameters(source_parameters<double>*,
                                            gen_params_base<double>*,
                                            std::string);

template <class T>
void cleanup_source_parameters(source_parameters<T>* params,
                               std::string generation_method) {
  if (check_theory_support(generation_method)) {
...
template void cleanup_source_parameters(source_parameters<adouble>*,
                                        std::string);
template void cleanup_source_parameters(source_parameters<double>*,
                                        std::string);
...
template <class T>
void assign_polarizations(std::string generation_method,
...
template void assign_polarizations<double>(std::string generation_method,
                                           waveform_polarizations<double>* wp);
template void assign_polarizations<adouble>(
    std::string generation_method, waveform_polarizations<adouble>* wp);
